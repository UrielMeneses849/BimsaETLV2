...
def _apply_styles_excel_and_sheets(ws, header_row: int, first_data_row: int, nrows: int, ncols: int, orig_headers=None):
    ...
    if orig_headers:
        # Do not touch header styles, only ensure alignment
...
def ETL_BIMSA(
    ...
    # =========================================================
    # NUEVO GENERADOR DE EXCEL BASADO EN PLANTILLA
    # =========================================================

    TEMPLATE_MAP = {
        "CLASICO": "templates/Clasico.xlsx",
        "PROSPECCION": "templates/Prospeccion.xlsx",
        "MAPAS": "templates/Mapas.xlsx",
        "CONTACTOS": "templates/Contactos.xlsx",
    }

    template_path = TEMPLATE_MAP.get(tipo_upper)

    if not template_path:
        raise ValueError("Tipo de reporte no soportado para plantillas")

    template_resolved = _resolve_resource_path(template_path)
    ...
    # Leer encabezados reales desde la plantilla
    template_headers = [
    ...
    for r_offset in range(total_rows):
        ...
        row = data_matrix[r_offset]

        for col_index in range(total_cols):
            ws.cell(excel_row, col_index + 1, row[col_index])
    ...
    # eliminar filas sobrantes debajo del último registro
    if ws.max_row > nrows:
        ws.delete_rows(nrows + 1, ws.max_row - nrows)
    ...
    _apply_styles_excel_and_sheets(
        ...
        nrows=nrows,
        ncols=ncols,
        orig_headers=template_headers if es_clasico else None
    )
